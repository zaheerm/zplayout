...
class Playlist(object):
    ...
    def addItem(self, piid, timestamp, uri, offset, duration,
        ...
        """
        current = self._getCurrentItem()
        factor = int(time.time() * gst.SECOND) - self.producer.pipeline.get_clock().get_time()
        translatedTs = timestamp - factor
        if current and translatedTs < current.timestamp + current.duration:
        ...
        newitem = PlaylistItem(piid, translatedTs, uri, offset, duration)
...
class PlaylistParser(object):
    ...
    def startDiscovery(self, doSort=True):
        ...
        @type  doSort: bool
        """
        self.debug('startDiscovery: discovering: %s, block: %d, pending: %d' %
        ...
        def _discoverer_done(disc, is_media):
            if is_media:
                ...
                if duration > 0:
                    ...
                    pass #self.warning("Duration of item is zero, not adding")
            else:
                pass #self.warning("Discover failed to find media in %s" % (item[0]))
